...
const CIRCUITS: Record<string, CircuitInfo> = {
  ...
  // ── bahrain ──
  // GP Bahrain 2026: ospitato eccezionalmente al Sepang International Circuit (Malesia)
  bahrain: {
    label: 'Sepang',
    startLine: [63.4, 91.7, 62.6, 83.7],
    d: `M 63.0,87.7 L 14.5,92.8 L 14.5,83.8 L 24.8,84.9 L 28.2,81.3 L 19.6,67.2 L 20.3,47.9 L 30.8,35.3 L 88.3,10.0 L 91.6,10.8 L 105.1,52.5 L 122.7,56.5 L 136.8,45.0 L 148.0,43.4 L 185.5,80.5 L 178.9,95.3 L 108.7,107.3 L 125.0,124.7 L 124.0,137.7 L 114.7,150.0 L 75.7,123.8 L 55.6,132.2 L 38.6,130.8 L 32.1,124.4 L 31.7,116.5 L 158.7,85.9 L 160.8,78.7 L 154.6,76.1 Z`,
    length: '5.543 km',
    laps: 56,
    distance: '310.4 km',
    record: { time: '1:34.080', driver: 'Sebastian Vettel', year: 2017 },
  ...
  saudi: {
    ...
    laps: 50,
    distance: '308.7 km',
    record: { time: '1:20.273', driver: 'Max Verstappen', year: 2024 },
  ...
  belgium: {
    ...
    laps: 44,
    distance: '308.2 km',
    record: { time: '1:44.701', driver: 'Sergio Perez', year: 2024 },
  ...
  singapore: {
    ...
    laps: 62,
    distance: '306.3 km',
    record: { time: '1:34.486', driver: 'Daniel Ricciardo', year: 2024 },
  ...
  lasvegas: {
    ...
    laps: 50,
    distance: '310.1 km',
    record: { time: '1:13.876', driver: 'Lando Norris', year: 2024 },
...
const CircuitMap: React.FC<CircuitMapProps> = ({
  ...
  if (showInfo && !showLabel) {
    // Info-only mode: no SVG — scheda tecnica compatta
    return (
      <div className={`space-y-1.5 ${className}`}>
        <div className="flex justify-between items-center">
          <span className="text-[9px] uppercase tracking-widest font-black text-white/25">Lunghezza</span>
          <span className="text-[11px] font-black text-white/70">{circuit.length ?? '—'}</span>
        </div>
        <div className="flex justify-between items-center">
          <span className="text-[9px] uppercase tracking-widest font-black text-white/25">Giri</span>
          <span className="text-[11px] font-black text-white/70">{circuit.laps ?? '—'}</span>
        </div>
        <div className="flex justify-between items-center">
          <span className="text-[9px] uppercase tracking-widest font-black text-white/25">Distanza</span>
          <span className="text-[11px] font-black text-white/70">{circuit.distance ?? '—'}</span>
        </div>
        {circuit.record && (
          <div className="pt-2 mt-1 border-t border-white/5">
            <p className="text-[9px] uppercase tracking-widest font-black text-white/25 mb-1">Record</p>
            <p className="text-[13px] font-black italic text-primary leading-none">{circuit.record.time}</p>
            <p className="text-[9px] font-bold text-white/40 mt-0.5">{circuit.record.driver} · {circuit.record.year}</p>
          </div>
        )}
      </div>
    );
